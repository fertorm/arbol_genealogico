...
function DPad({ onPan, onReset }) {
  ...
  const b = (dir, lbl) => (
    <div
      onMouseDown={()=>start(dir)} onMouseUp={stop} onMouseLeave={stop}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    document.addEventListener("touchstart", preventDblTap, { passive: false });
    return () => {
      document.removeEventListener("touchmove", preventZoom);
      document.removeEventListener("touchstart", preventDblTap);
    };
  }, []);
  ...
  const addMember = async () => {
    ...
    if(form.isPortal){
      ...
      const { data, error } = await supabase.from("members").insert({
        ...
        photo:form.photo, year:form.year, creator_id:MY_ID, ...basePos,
      }).select().single();
      if(error){ showToast("❌ Error: "+error.message); return; }
      if(data){ setMembers(p=>[...p,data]); membersRef.current=[...membersRef.current,data]; }
    }
