...
// No es criptografía fuerte, pero evita lectura casual en la DB
...
export default function SoltarConAmor({ member, myId, treeId, onClose }) {
  const [phase, setPhase] = useState("loading"); // loading | write | burn | ash | peace
  const [content, setContent] = useState("");
  const [savedLetter, setSavedLetter] = useState(null); // carta guardada en DB
  ...
  const burnLetter = async () => {
    setPhase("burn");
    // Guardar antes de quemar si hay contenido nuevo
    if (content.trim()) {
      const encrypted = encrypt(content, myId);
      if (savedLetter) {
        await supabase.from("letters").update({ content_encrypted: encrypted }).eq("id", savedLetter.id);
        ...
        await supabase.from("letters").insert({
          ...
          creator_id: myId, content_encrypted: encrypted, recipient_name: member.name,
        });
      }
    }
    // Animación burn → ash → peace
    ...
    <>
      <style>{`
        @keyframes flicker1 {
          ...
          25%{transform:scaleY(1.15) scaleX(0.92) rotate(1deg);opacity:1;}
          50%{transform:scaleY(0.95) scaleX(1.05) rotate(-0.5deg);opacity:0.85;}
          75%{transform:scaleY(1.1) scaleX(0.95) rotate(1.5deg);opacity:0.95;}
        }
        @keyframes flicker2 {
          0%,100%{transform:scaleY(0.9) scaleX(1.05) rotate(1deg);opacity:0.8;}
          33%{transform:scaleY(1.2) scaleX(0.9) rotate(-1.5deg);opacity:1;}
          66%{transform:scaleY(0.85) scaleX(1.08) rotate(0.5deg);opacity:0.75;}
        }
        @keyframes flicker3 {
          0%,100%{transform:scaleY(1.05) scaleX(0.95);opacity:0.6;}
          40%{transform:scaleY(0.8) scaleX(1.1) rotate(2deg);opacity:0.9;}
          70%{transform:scaleY(1.15) scaleX(0.88) rotate(-1deg);opacity:0.5;}
        }
        @keyframes burnPage {
          0%{opacity:1;transform:scaleY(1) translateY(0);filter:brightness(1);}
          20%{filter:brightness(1.3) sepia(0.3);}
          40%{filter:brightness(1.6) sepia(0.7) saturate(1.5);}
          60%{opacity:0.7;transform:scaleY(0.85) translateY(10px);filter:brightness(2) sepia(1) saturate(2);}
          80%{opacity:0.3;transform:scaleY(0.5) translateY(30px);filter:brightness(2.5) sepia(1);}
          100%{opacity:0;transform:scaleY(0) translateY(60px);filter:brightness(3) sepia(1);}
        }
        @keyframes ashFall {
          0%{opacity:1;transform:translateY(0) rotate(0deg);}
          100%{opacity:0;transform:translateY(80px) rotate(${Math.random()>0.5?'':'-'}${20+Math.random()*40}deg);}
        }
        @keyframes fadeInPeace {
        ...
        .flame-wrap { position:relative; display:flex; justify-content:center; align-items:flex-end; height:120px; }
        ...
        .smoke { position:absolute; bottom:110px; left:50%; transform:translateX(-50%); }
        .smoke-p { width:8px; height:8px; border-radius:50%; background:rgba(180,180,180,0.4); position:absolute; animation:rise 2s ease-out infinite; }
      ...
      <div
        ...
        style={{ position:"fixed",inset:0,background:"rgba(20,10,5,0.75)",backdropFilter:"blur(6px)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:"16px",fontFamily:"'Jost',sans-serif" }}>
        ...
        {/* ── FASE: LOADING ── */}
        {phase==="loading" && (
          <div style={{ color:"rgba(255,240,210,0.6)",fontSize:16,fontFamily:"'Cormorant Garamond',serif",letterSpacing:1 }}>
        ...
        {phase==="write" && (
          <div onClick={e=>e.stopPropagation()} style={{ width:"100%",maxWidth:500,maxHeight:"92vh",overflowY:"auto" }}>
            ...
            {/* Papel envejecido */}
            <div style={{ background:"linear-gradient(135deg,#FDF8F0 0%,#F9F1E2 40%,#F5EBD4 100%)",border:"1px solid rgba(180,140,80,0.3)",borderRadius:4,boxShadow:"0 0 60px rgba(255,100,0,0.15), 0 20px 60px rgba(0,0,0,0.5)",position:"relative",overflow:"hidden" }}>

              {/* Textura papel */}
              <div style={{ position:"absolute",inset:0,backgroundImage:"repeating-linear-gradient(0deg,transparent,transparent 27px,rgba(180,140,80,0.08) 28px)",pointerEvents:"none" }}/>

              {/* Header */}
              <div style={{ padding:"20px 24px 14px",borderBottom:"1px solid rgba(180,140,80,0.2)",display:"flex",alignItems:"center",gap:12 }}>
                <div style={{ fontSize:28 }}>💌</div>
                <div>
                  <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontWeight:400,color:"#3D2B1F",letterSpacing:0.5 }}>
                    Soltar con amor
                  </div>
              ...
              {/* Saludo */}
              <div style={{ padding:"16px 24px 0",fontFamily:"'Cormorant Garamond',serif",fontSize:16,fontWeight:400,color:"rgba(93,58,26,0.7)",fontStyle:"italic" }}>
              ...
              {/* Firma */}
              <div style={{ padding:"0 24px 8px",fontFamily:"'Cormorant Garamond',serif",fontSize:15,fontWeight:400,color:"rgba(93,58,26,0.6)",fontStyle:"italic",textAlign:"right" }}>
                Con amor,<br/>
                <span style={{ fontSize:13,color:"rgba(93,58,26,0.4)" }}>{new Date().toLocaleDateString("es-BO",{day:"numeric",month:"long",year:"numeric"})}</span>
              </div>

              {/* Info cifrado */}
              <div style={{ margin:"0 24px 12px",padding:"8px 12px",background:"rgba(139,111,71,0.08)",border:"1px solid rgba(139,111,71,0.15)",borderRadius:2,fontSize:10,color:"rgba(93,58,26,0.5)",display:"flex",gap:6,alignItems:"center" }}>
                <span>🔒</span>
              ...
              <div style={{ padding:"12px 24px 20px",display:"flex",gap:8,flexWrap:"wrap" }}>
                ...
                {savedLetter && content.trim()===decrypt(savedLetter.content_encrypted,myId) && (
                  <button onClick={deleteDraft}
                    style={{ padding:"10px 12px",background:"transparent",border:"1.5px solid rgba(180,60,60,0.3)",borderRadius:2,fontFamily:"'Jost',sans-serif",fontSize:11,color:"rgba(180,60,60,0.7)",cursor:"pointer",letterSpacing:"0.8px",textTransform:"uppercase" }}>
                    🗑️
                  </button>
                )}
                <button onClick={burnLetter} disabled={!content.trim()}
                  style={{ flex:2,padding:"10px 6px",background:content.trim()?"linear-gradient(135deg,#CC3300,#FF6600)":"rgba(139,111,71,0.15)",border:"none",borderRadius:2,fontFamily:"'Jost',sans-serif",fontSize:11,color:content.trim()?"#FFF8F0":"rgba(93,58,26,0.3)",cursor:content.trim()?"pointer":"default",letterSpacing:"1px",textTransform:"uppercase",fontWeight:500,boxShadow:content.trim()?"0 4px 20px rgba(255,100,0,0.3)":"none",transition:"all 0.3s" }}>
                  🔥 Quemar y soltar
                </button>
              </div>

              {savedLetter && (
                <div style={{ padding:"0 24px 14px",fontSize:10,color:"rgba(139,111,71,0.5)",textAlign:"center" }}>
        ...
        {phase==="burn" && (
          <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:0,width:"100%",maxWidth:500 }}>
            ...
            <div className="burn-text"
              ...
              <div style={{ position:"relative",zIndex:1 }}>{content.slice(0,120)}{content.length>120?"...":""}</div>
            </div>

            {/* Llamas */}
            <div className="flame-wrap" style={{ width:"100%" }}>
              <div className="smoke">
                {[...Array(5)].map((_,i)=>(
                  <div key={i} className="smoke-p" style={{ left:`${-20+i*10}px`,animationDelay:`${i*0.4}s`,animationDuration:`${1.5+i*0.3}s` }}/>
                ))}
              </div>
            ...
            <div style={{ color:"rgba(255,200,100,0.7)",fontSize:13,letterSpacing:"1px",marginTop:8,fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic",animation:"shimmer 1.5s ease-in-out infinite" }}>
        ...
        {phase==="ash" && (
          <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:20,width:"100%",maxWidth:400 }}>
            <div style={{ position:"relative",height:120,width:300,display:"flex",alignItems:"flex-end",justifyContent:"center" }}>
              {/* Llamas pequeñas apagándose */}
              <div style={{ opacity:0.3 }}>
                <div className="flame" style={{ width:20,height:30,background:"linear-gradient(to top,#FF4500,#FFD700)",animation:"flicker1 0.6s ease-in-out infinite",left:"50%",marginLeft:-10,position:"absolute",bottom:20 }}/>
              </div>
              {/* Cenizas cayendo */}
              {[...Array(12)].map((_,i)=>(
              ...
              {/* Montículo de ceniza */}
              <div style={{ width:80,height:12,background:"radial-gradient(ellipse,rgba(100,100,100,0.5),transparent)",borderRadius:"50%",position:"absolute",bottom:0 }}/>
            </div>
            <div style={{ color:"rgba(200,180,150,0.7)",fontSize:14,letterSpacing:"0.5px",fontFamily:"'Cormorant Garamond',serif",fontStyle:"italic" }}>
        ...
        {phase==="peace" && (
          <div onClick={onClose}
            ...
            <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:17,fontStyle:"italic",color:"rgba(255,220,150,0.75)",lineHeight:1.7,padding:"0 16px" }}>
              "{frase}"
            </div>

            <div style={{ width:60,height:1,background:"rgba(255,200,100,0.3)" }}/>

            <div style={{ fontSize:11,color:"rgba(255,200,100,0.4)",letterSpacing:"1px",textTransform:"uppercase" }}>
              Tu carta ha sido liberada para siempre
            </div>

            <div style={{ fontSize:11,color:"rgba(255,200,100,0.25)",marginTop:8 }}>
              Toca para cerrar
            </div>
          </div>
        )}
